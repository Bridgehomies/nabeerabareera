...
export default function SalePage() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        const saleOnly = processed.filter((p: any) => p.isSale)
        const featured = saleOnly.filter((p: any) => p.featured || p.rating >= 4.5)
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-screen">Loading products...</div>
    )
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen text-red-600">
        ...
        <button
          ...
          className="mt-4 underline hover:text-red-800"
        >
          Try Again
        </button>
      </div>
